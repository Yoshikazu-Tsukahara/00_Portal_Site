...
const CACHE_NAME = "pixel-drop-puzzle-v4";
const PRECACHE = [
  "/pixel-drop-puzzle",
...
self.addEventListener("fetch", (event) => {
  ...
  const inScope =
    ...
    path.startsWith("/pixel-drop-puzzle/") ||
    path.startsWith("/icons/pixel-drop-puzzle");
  if (!inScope) return;
    ...
    event.respondWith(
      fetch(request)
        ...
        .catch(() =>
          caches
            .match(request)
            .then((cached) => cached || caches.match("/pixel-drop-puzzle")),
        ),
    );
    return;
  }
