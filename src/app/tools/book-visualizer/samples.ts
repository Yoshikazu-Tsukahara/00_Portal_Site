...
function sampleNovel(): BookData {
  ...
  const ch4s1 = joinJa(
    "ある夜、吾輩は夢を見た。広い河原に名前が散らばっていて、風がそれを紙吹雪のように運んでいく。吾輩はそのうちの一つを咥えようとして、何度も失敗した。起きてみると、障子の外は本当に風が強かった。",
    "朝になると庭に柿の葉が落ちていた。吾輩はその傍で身繕いをし、ふと主人の靴音を聞いた。主人は吾輩を見て、珍しく短い言葉を残した。「今日も元気だな」。名前ではない。しかし名前より柔らかいものが、たしかに届いた。",
    "吾輩は応えるように喉を鳴らし、縁側の日向へ戻った。呼ばれずとも、認められる瞬間はある。その瞬間を、吾輩は大切に畳へしまっておくことにした。",
  );
  ...
  const ch4s2 = joinJa(
    ...
    "読者よ、もし貴方が人間なら、どうか尻尾を踏まないでいただきたい。そして可能なら、縁側に日向を残しておいていただきたい。それが吾輩からの、静かなお願いである。",
  );
...
function sampleWestern(): BookData {
  ...
  const c4a = joinEn(
    "Years later—or perhaps only pages later—{{name1}} tried to describe the library to a friend and failed in the gentlest way. Every accurate detail sounded invented. Every invented detail sounded true. That, {{name2}} would have said, is how quiet buildings travel: as rumors that insist on being visited.",
    "Still, {{name1}} kept a private map. Not of streets, but of temperatures: the cool of unread pages, the warmth of a returned hour, the exact gray of rain that makes a crooked sign look deliberate. On difficult mornings {{name1}} would open a notebook and write one ordinary sentence, the way the untitled volume had once required.",
    "Begin where you are. Stay until the quiet answers. Return what you can. Leave the walls standing.",
  );

  const coda = joinEn(
    "Author’s note: This sample is fiction for layout practice in the page editor. Placeholders like {{name1}} are replaced when a reader opens the book—pagination runs after substitution so line breaks stay accurate. Use it to check chapter headers, section depth in the TOC, and manual page breaks across a trade paperback spread.",
  );
  ...
  return {
    ...
    body: [
      ...
      para(c3b),
      breakPage(),
      heading("h1", "Chapter IV · After the Rain"),
      heading("h2", "A Private Map of Temperatures"),
      para(c4a),
      breakPage(),
      heading("h1", "Coda"),
      para(coda),
...
/**
 * サンプル3：正方形・写真集寄りのレイアウト
 * フルブリード画像ページ＋短いエッセイ本文。柱はタイトル、目次は章のみ。
 */
function samplePhoto(): BookData {
  const essay1 = joinJa(
    "旅は地図より先に、光の温度で始まる。朝のプラットホーム、窓に映る自分の輪郭、まだ誰の記憶にもなっていない景色。カメラを構える指が、わずかに震えるのは緊張ではない。世界がこちらを見返している気配への、礼儀のようなものだ。",
    "この写真集は、特定の地名を名乗らない。代わりに、色と影の順序だけを残した。青が濃くなる駅、白い埃の港、灯りの下で金色に戻る食卓。場所の名前より、そこにいた時間の厚みを優先した。",
    "{{traveler}}が初めてシャッターを切った日、風は西から来た。二枚目は失敗だった。三枚目でようやく、光が被写体の輪郭を許した。失敗の枚数まで含めて旅だと、あとから気づく。",
  );

  const essay2 = joinJa(
    "昼下がりの坂道は、足音を長くする。壁の漆喰、錆びた手すり、乾いた洗濯物。どれも特別ではないのに、並べると物語になる。写真は「すごいもの」を撮る装置ではなく、「見過ごしそうなもの」を残す装置なのかもしれない。",
    "誰かがフレームの端を横切る。顔は写らない。それでも、その一瞬の速度が画面を生きものにする。完璧な構図より、わずかな乱れの方が、後から見返したときに体温を持っている。",
    "{{companion}}と並んで歩いた午後は、会話より沈黙の方が多かった。沈黙は空白ではない。シャッターの間隔に似ている。次の一枚まで、世界をいったん預けておく時間だ。",
  );

  const essay3 = joinJa(
    "夜になると、街は輪郭を手放し、点の集合になる。ネオン、窓明かり、遠くの橋。露出を長めにすると、光は線になり、時間はにじむ。止まっているはずの写真が、じつは動いている痕跡を抱えていることに気づく。",
    "最後の章は、帰宅の手前で撮った。荷物のファスナー、チケットの折れ目、靴底の砂。壮大な景色より、旅の終わりを告げる小物の方が、アルバムの背骨になることがある。",
    "もしこの本を手に取ったなら、どこかで同じ色の光を見つけてほしい。名前のない旅でも、見る人の記憶には、ちゃんと住所が生まれるはずだから。",
  );
  ...
  const plate = (
    ...
    page(
      "standard",
      imageFullBleed(placeholderImage(label, from, to, 720, 720)),
      freeText(
        caption,
        { x: 0.08, y: 0.78, w: 0.84, h: 0.08 },
        0.028,
        "horizontal",
  ...
  return {
    ...
    format: {
      ...
      tocDepth: "chapter",
      tocColumns: 1,
      fontFamilyH1: "noto-serif-jp",
      fontFamilyH2: "noto-sans-jp",
      fontFamilyP: "noto-serif-jp",
    },
    body: [
      heading("h1", "Ⅰ　朝の輪郭"),
      para(essay1),
    ...
    pages: [
      page(
        "cover",
        imageFullBleed(
          placeholderImage("LIGHT", "#1c1917", "#a8a29e", 720, 720),
        ),
        freeText(
          "無名の光録",
          { x: 0.1, y: 0.38, w: 0.8, h: 0.12 },
          0.05,
          "horizontal",
          { fontFamily: "noto-serif-jp" },
        ),
        freeText(
          "A square photo-book sample",
          { x: 0.1, y: 0.54, w: 0.8, h: 0.08 },
          0.022,
          "horizontal",
          { fontFamily: "noto-sans-jp" },
        ),
      ...
      page("toc"),
      plate("01", "#0f172a", "#334155", "Plate 01　早朝のホーム", "Blue hour / Station"),
      plate("02", "#14532d", "#4d7c0f", "Plate 02　港の白い埃", "Harbor dust"),
      plate("03", "#7c2d12", "#c2410c", "Plate 03　坂の漆喰", "Afternoon slope"),
      plate("04", "#312e81", "#6366f1", "Plate 04　夜の点描", "City as points"),
      plate("05", "#44403c", "#d6d3d1", "Plate 05　帰宅の手前", "Before return"),
      page(
        "backCover",
        imageFullBleed(
          placeholderImage("END", "#0c0a09", "#57534e", 720, 720),
        ),
        freeText(
          "名前のない旅にも、見る人の記憶には住所が生まれる。",
          { x: 0.12, y: 0.42, w: 0.76, h: 0.16 },
          0.026,
          "horizontal",
          { fontFamily: "noto-serif-jp" },
        ),
