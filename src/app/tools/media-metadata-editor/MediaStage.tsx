...
/** 左カラム：音楽はジャケット、動画はプレイヤー＋フレームキャプチャ */
  ...
  return (
    <section className="flex h-full min-h-0 flex-col gap-4">
      <div className="min-w-0">
        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-amber-500/90">
          {mode === "audio" ? copy.modeAudio : copy.modeVideo}
        </p>
        {/* 長い名前は横スクロールせず最大2行で折り返し */}
        <h2 className="mt-1 line-clamp-2 break-all text-sm font-semibold leading-snug text-zinc-100">
          {fileName}
        </h2>
        {fileMeta ? (
      ...
      {mode === "audio" ? (
        <>
          <div className="relative flex min-h-[16rem] flex-1 items-center justify-center overflow-hidden rounded-2xl border border-zinc-800 bg-gradient-to-b from-zinc-900 to-zinc-950">
            {artwork.previewUrl ? (
              ...
              <div className="flex flex-col items-center gap-2 text-zinc-600">
                <ImagePlus className="h-10 w-10 opacity-50" aria-hidden />
          ...
          >
            <p className="text-sm font-medium text-zinc-200">{copy.artworkDrop}</p>
            <p className="mt-1 text-xs text-zinc-500">{copy.artworkDropSub}</p>
            <p className="mt-2 text-[11px] text-zinc-600">{copy.artworkHint}</p>
            <input
              ref={artInputRef}
              type="file"
        ...
        <>
          ...
          <button
            ...
            disabled={capturing}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-60"
          >
            <Clapperboard className="h-4 w-4" aria-hidden />
            {capturing ? copy.capturing : copy.captureFrame}
          </button>
          {captureError ? (
            <p className="text-xs text-rose-400">{captureError}</p>
          ) : null}

          <div className="min-h-[8rem] overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/60">
            <p className="border-b border-zinc-800 px-3 py-2 text-[11px] font-medium uppercase tracking-wider text-zinc-500">
              {copy.thumbPreview}
            </p>
            <div className="flex min-h-[7rem] items-center justify-center p-3">
              {artwork.previewUrl ? (
                ...
                <p className="text-xs text-zinc-600">{copy.noArtwork}</p>
              )}
            </div>
          </div>
        </>
