...
export default function InstallGuideModal({
  ...
  copy: PixelDropPuzzleDict["install"];
  onClose: () => void;
}) {
  const titleId = useId();
  if (!open) return null;
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm overflow-hidden rounded-t-3xl border border-zinc-800 bg-zinc-950 font-mono text-zinc-100 shadow-2xl sm:rounded-3xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="relative px-5 pb-2 pt-5 text-center">
          <div
            className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-400/10 text-2xl"
            aria-hidden
          >
            🧩
          </div>
        ...
        <div className="border-t border-zinc-800 px-5 py-4">
          <button
            type="button"
            onClick={onClose}
            className="w-full rounded-lg border border-zinc-700 py-3 text-sm font-medium text-zinc-300 transition-colors hover:border-zinc-500 hover:text-zinc-100"
          >
            {copy.modalClose}
          </button>
        </div>
