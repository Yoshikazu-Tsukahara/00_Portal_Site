...
type HomeModeProps = {
  ...
  hasDraft: boolean;
  draftTitle: string;
  /** true のとき作成・編集導線を出さず、ファイル読み込みのみ */
  readOnly?: boolean;
  onCreate: () => void;
  onResume: () => void;
  /** サンプルを読み込んで編集を開く */
  ...
  const readSection = (
    <section className="flex min-w-0 flex-col gap-2">
      <div
        ...
        onDrop={handleDrop}
        className={`bv-dropzone flex min-h-[12rem] flex-1 flex-col items-center justify-center gap-3 rounded-2xl bg-white/60 p-5 text-center sm:min-h-[16rem] sm:p-8 ${
      ...
      >
        ...
        <p className="max-w-sm break-words text-xs leading-relaxed text-[#6f665b]">
          {readOnly ? copy.home.readOnlyLead : copy.home.readLead}
        </p>
        <button
          type="button"
