...
/** ホーム画面追加の操作ガイド（iOS / PC・Android 共通枠・ダーク配色） */
export default function InstallGuideModal({
  ...
  const steps = isIos
    ? [
        { n: 1, title: copy.step1Title, body: copy.step1Body, icon: <ShareIcon /> },
        { n: 2, title: copy.step2Title, body: copy.step2Body, icon: <HomeAddIcon /> },
      ]
    : [
        {
          n: 1,
  ...
  return (
    ...
    >
      <div
        className="slot-install-modal w-full max-w-sm overflow-hidden rounded-t-3xl border border-zinc-800 bg-zinc-950 font-mono text-zinc-100 shadow-2xl sm:rounded-3xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="relative px-5 pb-2 pt-5 text-center">
          <div
            className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-400/10 text-2xl"
            aria-hidden
          >
            🎰
          </div>
        ...
        <ol className="space-y-3 px-5 py-4">
          {steps.map((s) => (
            <Step key={s.n} n={s.n} title={s.title} body={s.body} icon={s.icon} />
          ))}
        </ol>
        ...
        <div className="border-t border-zinc-800 px-5 py-4">
          <button
            type="button"
            onClick={onClose}
            className="slot-ghost-btn w-full !py-3 !text-sm"
          >
            {copy.modalClose}
          </button>
        </div>
...
function Step({
  ...
  return (
    <li className="flex gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/60 px-3.5 py-3">
      <span
        className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-amber-400 text-xs font-semibold text-zinc-950"
        aria-hidden
      >
        {n}
      </span>
