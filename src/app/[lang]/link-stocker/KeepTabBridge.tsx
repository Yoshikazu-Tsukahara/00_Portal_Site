...
function isKeepAppPath(pathname: string | null): boolean {
  ...
  return bare === "/link-stocker" || bare.startsWith("/link-stocker/");
}
...
function isBridgePath(pathname: string | null): boolean {
  ...
  const bare = stripLocalePrefix(pathname);
  return (
    bare === "/link-stocker/bridge" ||
    bare.startsWith("/link-stocker/bridge/")
  );
}
  ...
  useEffect(() => {
    ...
    try {
      ...
      channel.onmessage = (ev: MessageEvent<KeepRequestMsg>) => {
        ...
        if (!query) return;
        const locale = getLocaleFromPathname(path) ?? DEFAULT_LOCALE;
        window.location.assign(
          `${localizedHref(locale, "/link-stocker")}${query}`,
        );
      };
    } catch {
