...
export const ko: PartialDictionary = {
  ...
  tools: {
    ...
    "character-relation-editor": {
      title: "소설 관계도",
      description: "등장인물 카드와 관계선으로 이야기의 상관도를 시각적으로 정리.",
    },
    "book-visualizer": {
      title: "지면 편집",
  ...
  footer: {
    ...
    noticeLabel: "주의사항",
    localOnly:
      "🔒 도구의 파일·입력 내용은 원칙적으로 브라우저에서 처리·저장되며, 운영자가 내용을 수집·보관하지 않습니다. 개인 추적 Cookie도 사용하지 않습니다. 사이트 개선을 위해 익명화된 방문·도구 이용 횟수만 집계합니다. 일부 도구는 제한된 네트워크 요청이 있을 수 있습니다(개인정보 처리방침 참고).",
  },
  messages: {
    environment:
      "도구는 원칙적으로 브라우저에서 동작하며 앱 설치가 필요 없습니다. Windows / Mac / 스마트폰 등에서 이용할 수 있지만, 일부는 PC 전용(모바일 미지원)입니다. 사이트 열람·최초 이용에는 인터넷이 필요하며, 사이트 전체 오프라인은 보장하지 않습니다.",
    persistence:
      "저장 데이터는 브라우저 LocalStorage(기기 내 임시 영역)에 둡니다. 캐시 삭제나 사이트 데이터 삭제, 다른 브라우저·기기로 바꾸면 사라질 수 있습니다. 중요한 데이터는 정기적으로 「내보내기(저장)」해 두세요.",
    safety:
      "도구에 입력·작성한 데이터는 원칙적으로 기기(브라우저) 안에만 저장되며, 운영자가 서버에 내용을 수집·보관하지 않습니다. 기기 측 위험까지는 막지 못하므로 중요한 데이터는 백업해 주세요.",
    safetyShort: "데이터는 원칙적으로 브라우저에 저장되며 운영자가 내용을 수집하지 않습니다.",
    privacyBanner:
      "파일·입력 내용은 원칙적으로 이 브라우저에서 처리됩니다. 운영자가 내용을 수집·저장하지 않습니다.",
    privacyBannerShort:
      "원칙적으로 브라우저 내 처리. 운영자는 입력을 저장하지 않습니다.",
  },
  dataManager: {
    ...
    export: "📥 데이터 내보내기(저장)",
    import: "📤 데이터 불러오기(로드)",
    noData: "이 도구는 세션 내에서만 동작하며 저장할 설정 데이터가 없습니다. 운영자가 처리 내용을 수집·저장하지도 않습니다.",
