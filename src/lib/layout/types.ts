/** サイト全体の表示幅モード（PC 向け。狭い画面では自動でコンパクト表示） */
 ...
 * 旧「portrait」は標準幅へ移行する。
 */
export const LAYOUT_MODE_BOOTSTRAP_SCRIPT = `(function(){try{var k=${JSON.stringify(LAYOUT_MODE_STORAGE_KEY)};var m=localStorage.getItem(k);if(m==="default"||m==="wide"||m==="full"){document.documentElement.dataset.${LAYOUT_MODE_DATASET_ATTR}=m;}else{if(m==="portrait"){try{localStorage.setItem(k,"default");}catch(e){}}document.documentElement.dataset.${LAYOUT_MODE_DATASET_ATTR}="default";}}catch(e){document.documentElement.dataset.${LAYOUT_MODE_DATASET_ATTR}="default";}})();`;
